    ...
    >
      <main className="space-y-8 w-full sm:w-auto p-4">
        ...
        <div className="text-gray-700 font-semibold text-lg space-y-2 flex flex-col items-center">
          <Image src={Logo} alt="docify" width={62} height={62} />
          <h1>Docify</h1>
        </div>

        <button
          onClick={handleClick}
          className="bg-gray-200 flex items-center gap-1 rounded-xl mx-auto text-gray-500 text-sm p-2 px-4"
        >
          Create new document
        ...
        <div className="space-y-2">
          ...
          {documents.length > 0 && (
            <div className="py-2 px-4 space-y-4 rounded-xl bg-gray-100 sm:min-w-96">
              {documents.map((doc) => (
                <motion.div layout key={doc.id} className={"py-2"}>
